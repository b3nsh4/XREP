...
def patt1_static_str(
	...
	if len(lhs)!=0 and len(rhs)!=0:
		...
		if pyre==True:
			res = f"re.findall(\"{quantifier}{pre1}{pre2}{p_space}({cooked_string}).*{post1}.*{post2}\",TXT)"
			return res
		elif pyre==False:
		...
		if pyre==True:
			res = f"re.findall(\"{init_with}{p_space}{(cooked_string)}.*{post1}{post2}\",TXT)"
			return res
		elif pyre==False:
		...
		if pyre==True:
			res  = f"re.findall(\"{quantifier}{pre1}{pre2}{p_space}({cooked_string}).*\",TXT)"
			return res
